...
function renderResults() {
    ...
    pageDocs.forEach((data, index) => {
        ...
        resultRow.innerHTML = `
            <td>
                <div style="font-weight: 700; color: var(--primary-dark);">${data.encontro || 'Sem nome'}</div>
            </td>
            <td>${formattedDate}</td>
            <td>
...
async function printResult(id) {
    try {
        const docSnap = await getDoc(doc(db, 'encontros', id));
        if (docSnap.exists()) {
                ...
                <div class="service-order">
                    ...
                    <div class="content">
                        <div class="section">
                            ...
                            <p><strong>Encontro:</strong> ${data.encontro}</p>
