...
async function exportarParaPDF() {
    try {
        const celebracoesRef = collection(db, "encontros");
        ...
        // Cabeçalho
        doc.setFontSize(18);
        doc.text("Relatório de Encontros", 15, 15);
        doc.setFontSize(10);
        ...
        doc.autoTable({
            head: [['Data Criação', 'Data Celebração', 'Faltantes', 'Encontro']],
            body: body,
            startY: 25,
            theme: 'grid',
            styles: { fontSize: 10 },
        ...
        // Salvar PDF
        doc.save(`encontros_${new Date().toISOString().split('T')[0]}.pdf`);
